...
const { isRetryableValidationError, shouldRetryDownload } = require("../services/DownloadManager");
...
test("shouldRetryDownload retries large-file decode failures when expected size is known", () => {
	assert.equal(
		shouldRetryDownload("ffmpeg sampled decode: 1/4 passed; fatal: truncated file", LARGE_FILE_SIZE, null),
		true,
	);
...
test("shouldRetryDownload retries large-file decode failures when only observed size is known", () => {
	assert.equal(
		shouldRetryDownload("ffmpeg sampled decode: 0/5 passed; fatal: no frame found", null, LARGE_FILE_SIZE),
		true,
	);
});
...
test("shouldRetryDownload does not retry small-file decode failures or non-retryable errors", () => {
	assert.equal(
		shouldRetryDownload("ffmpeg sampled decode: 0/5 passed; fatal: no frame found", null, SMALL_FILE_SIZE),
		false,
	);
