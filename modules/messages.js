			...
			for (let i = 0; i < messages.length; i++) {
				...
				if (message.media) {
					...
					if (
						...
						promArr.push(
							downloadMessageMedia(client, message, mediaPath)
						);
					} else {
						fileExist
							? logMessage.info(
					...
					if (promArr.length === MAX_PARALLEL_DOWNLOAD) {
						...
						function WaitFirst(promArr) {
							return new Promise((resolve, reject) => {
								Promise.race(
									promArr.map((prom, i) => {
										return new Promise((res, rej) =>
											prom.then(() => res(i)).catch(rej)
										);
									})
								)
									.then(resolve)
						...
						const compIndex = await WaitFirst(promArr);
						// logMessage.success(
						// 	`File ${compIndex} downloaded successfully, adding more files`
						// );
						promArr = promArr.splice(compIndex, 1);
					}
				}
			}
			if (promArr.length > 0) {
...
const getMessageDetail = async (client, channelId, messageIds) => {
	try {
		...
		for (let i = 0; i < result.length; i++) {
			...
			if (message.media) {
				promArr.push(
					downloadMessageMedia(client, message, outputFolder)
				);
			}
			if (promArr.length === MAX_PARALLEL_DOWNLOAD) {
				logMessage.info(
